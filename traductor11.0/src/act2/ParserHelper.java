...
public class ParserHelper {
    ...
    public static class ExprResult {
        public String tipo;
        public Object valor;
        public String direccion; // Para generacion de codigo (ej. "t1", "x", "10")

        public ExprResult(String tipo, Object valor) {
            this.tipo = tipo;
            this.valor = valor;
            this.direccion = (valor != null) ? valor.toString() : "";
        }

        public ExprResult(String tipo, Object valor, String direccion) {
            this.tipo = tipo;
            this.valor = valor;
    ...
    public void insertarSimbolo(String id, String tipo, int linea, int columna, Object valor) {
        ...
        if (tablaDeSimbolos.peek().containsKey(id)) {
            reportSemanticError(
                    "Identificador '" + id + "' ya declarado en este ambito",
                    linea, columna);
        } else {
     ...
     * @param linea      Linea de la llamada
    ...
    public String validarLlamadaFuncion(String nombre, java.util.List<ExprResult> argumentos, int linea, int columna) {
        ...
        if (info == null) {
            reportSemanticError("Funcion '" + nombre + "' no declarada", linea, columna);
            return T_ERROR;
        }
            ...
            if (!tiposCompatibles(info.tiposParametros.get(i), argumentos.get(i).tipo)) {
                reportSemanticError(
                        "Tipo incompatible en argumento " + (i + 1) + " de '" + nombre + "'. " +
                                "Se esperaba '" + info.tiposParametros.get(i) + "' pero se recibió '"
    ...
    public boolean tiposCompatibles(String destino, String fuente) {
        ...
        // Mismo tipo
        if (destino.equals(fuente))
            return true;

        // Promoción implícita: int a float
        if (destino.equals(T_FLOAT) && fuente.equals(T_INT))
            return true;
    ...
    public String tipoResultanteAritmetico(String tipo1, String tipo2) {
        if (tipo1.equals(T_FLOAT) || tipo2.equals(T_FLOAT))
            return T_FLOAT;
        if (tipo1.equals(T_INT) && tipo2.equals(T_INT))
            return T_INT;
        return T_ERROR;
    }
     ...
     * @param linea    Linea del operador
    ...
    public String validarOperacionAritmetica(String tipo1, String tipo2, String operador, int linea, int columna) {
        // Ignora si algún operando ya es error
        if (tipo1.equals(T_ERROR) || tipo2.equals(T_ERROR))
            return T_ERROR;
    ...
    public void reportSyntaxError(String msg, Symbol tok) {
        erroresSint++;
        String m = "ERROR SINTACTICO: " + msg;
        if (tok != null && tok.value != null)
            m += " '" + tok.value + "'";
        if (tok != null)
            m += " en línea " + (tok.left + 1) + ", columna " + (tok.right + 1);
     ...
     * @param columna Número de columna (1-based)
     */
    public void reportLexicalError(String msg, int linea, int columna) {
        erroresLex++;
    ...
    public void imprimirTablaSimbolos() {
        ...
        System.out.println(String.format("%-25s %-12s %-8s %-8s %-30s %-15s",
                "Simbolo", "Tipo", "Linea", "Columna", "Alcance", "Valor"));
        ...
        for (SymbolInfo info : todosLosSimbolos) {
            ...
            String alcanceMostrado;
            if ("global".equals(info.alcance)) {
                alcanceMostrado = "Global";
            } else if (info.alcance.startsWith("local_")) {
                String nombreFuncion = info.alcance.substring(6);
                alcanceMostrado = "Local(" + nombreFuncion + ")";
            } else {
                alcanceMostrado = info.alcance;
            }
            ...
            if (info.esFuncion) {
                ...
                valorMostrado = info.valor != null ? info.valor.toString() : "null";
            }

            // Imprimir fila
            System.out.println(String.format("%-25s %-12s %-8d %-8d %-30s %-15s",
                    nombreSimbolo,
                    info.tipo,
                    info.linea,
                    info.columna,
