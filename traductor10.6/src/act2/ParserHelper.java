...
public class ParserHelper {
    ...
    public static class SymbolInfo {
        ...
        public SymbolInfo(String tipo, String alcance, int linea, int columna, Object valor) {
            ...
            this.valor = valor;
            this.nombre = null; // Se asignará desde insertarSimbolo
    ...
    info.nombre = id; // Guardamos el nombre en el SymbolInfo
    ...
    public SymbolInfo registrarFuncion(String nombre, String tipoRetorno, int linea, int columna) {
        ...
        info.nombre = nombre; // Guardamos el nombre en el SymbolInfo
        info.esFuncion = true;
        todosLosSimbolos.add(info);
    ...
    public void imprimirTablaSimbolos() {
        ...
        // Usamos todosLosSimbolos que contiene todos los símbolos registrados
        // Primero imprimimos símbolos globales
        System.out.println("=== Símbolos Globales ===");
        for (SymbolInfo info : todosLosSimbolos) {
            if ("global".equals(info.alcance)) {
                String nombreSimbolo = obtenerNombreSimbolo(info);
                if (info.esFuncion) {
                    System.out.printf("  %s -> Tipo: %s, Línea: %d, Columna: %d, Alcance: Global\n",
                            nombreSimbolo, info.tipo, info.linea, info.columna);
                } else {
                    System.out.printf("  %s -> Tipo: %s, Línea: %d, Columna: %d, Alcance: Global, Valor: %s\n",
                            nombreSimbolo, info.tipo, info.linea, info.columna,
        ...
        // Luego imprimimos símbolos locales de cada función
        for (SymbolInfo funcInfo : todosLosSimbolos) {
            if (funcInfo.esFuncion && !"global".equals(funcInfo.alcance)) {
                String nombreFuncion = obtenerNombreSimbolo(funcInfo);
                System.out.printf("\n=== Símbolos Locales (función %s) ===\n", nombreFuncion);
                
                // Imprimimos parámetros
                for (int i = 0; i < funcInfo.nombresParametros.size(); i++) {
                    System.out.printf("  %s -> Tipo: %s, Línea: %d, Columna: %d, Alcance: Local(%s)\n",
                            funcInfo.nombresParametros.get(i),
                            funcInfo.tiposParametros.get(i),
                            funcInfo.linea,
                            funcInfo.columna,
                            nombreFuncion);
                }
                
                // Imprimimos variables locales
                String alcanceFuncion = "local_" + nombreFuncion;
                for (SymbolInfo varInfo : todosLosSimbolos) {
                    if (alcanceFuncion.equals(varInfo.alcance) && !varInfo.esFuncion &&
                        !funcInfo.nombresParametros.contains(obtenerNombreSimbolo(varInfo))) {
                        String nombreVar = obtenerNombreSimbolo(varInfo);
                        System.out.printf("  %s -> Tipo: %s, Línea: %d, Columna: %d, Alcance: Local(%s), Valor: %s\n",
                                nombreVar, varInfo.tipo, varInfo.linea, varInfo.columna,
                                nombreFuncion,
                                varInfo.valor != null ? varInfo.valor : "null");
                    }
                }
            }
        }
        
        // Si hay funciones globales, imprimirlas
        for (SymbolInfo info : todosLosSimbolos) {
            if (info.esFuncion && "global".equals(info.alcance)) {
                String nombreFuncion = obtenerNombreSimbolo(info);
                System.out.printf("\n=== Símbolos Locales (función %s) ===\n", nombreFuncion);
                ...
                // Imprimimos variables locales de esta función
                String alcanceFuncion = "local_" + nombreFuncion;
                for (SymbolInfo varInfo : todosLosSimbolos) {
                    if (alcanceFuncion.equals(varInfo.alcance) && !varInfo.esFuncion &&
                        !info.nombresParametros.contains(obtenerNombreSimbolo(varInfo))) {
                        String nombreVar = obtenerNombreSimbolo(varInfo);
                        System.out.printf("  %s -> Tipo: %s, Línea: %d, Columna: %d, Alcance: Local(%s), Valor: %s\n",
                                nombreVar, varInfo.tipo, varInfo.linea, varInfo.columna,
                                nombreFuncion,
     ...
     */
    private String obtenerNombreSimbolo(SymbolInfo info) {
        if (info.nombre != null) {
            return info.nombre;
        }
